...
def generate_rhino_script(model_name: str, inputs: dict):
    ...
    *** This version is corrected to use int() for all 'range()' loops
    *** to fix the 'float' error that stopped the script after floor 1. ***
...
def build_model():
    ...
    if CORRIDOR_WIDTH > 0:
        ...
        # *** FIXED: Use int() to ensure whole number for range() ***
        num_units_per_floor = int(math.floor(rentable_area_per_floor / unit_size)) if unit_size > 0 else 0
        ...
        for i in range(N_STORIES):
            ...
            if CORRIDOR_POS == 'M':
                ...
                # *** FIXED: Use int() for range() ***
                num_units_per_side = int(math.floor(num_units_per_floor / 2))
                ...
                # *** FIXED: Use int() for range() ***
                num_units_on_side = int(num_units_per_floor)
